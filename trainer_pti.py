...
from krona import DiffuseKronAAttnProcessor
...
def main(
    ...
    for text_encoder in text_encoders:
        ...
        for name, attn_processor in unet.attn_processors.items():
            ...
            module = DiffuseKronAAttnProcessor(
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if global_step % checkpointing_steps == 0:
                ...
                krona_tensors = unet_attn_processors_state_dict(unet)

                save_file(
                    krona_tensors,
                    f"{checkpoint_dir}/unet/checkpoint-{global_step}.krona.safetensors",
                )
    ...
    save_file(
        krona_tensors,
        f"{output_dir}/krona.safetensors",
    )

    print(f"Krona tensors saved to {output_dir}/krona.safetensors")

    embedding_handler.save_embeddings(
        f"{output_dir}/embeddings.pti",
    )

    print(f"Embeddings saved to {output_dir}/embeddings.pti")

    to_save = token_dict
    with open(f"{output_dir}/special_params.json", "w") as f:
        json.dump(to_save, f)

    print(f"Special params saved to {output_dir}/special_params.json")
